...
def index_dictionary(path_to_tsv_file: str) -> dict:
	...
	with open(path_to_tsv_file, "r") as indexes_file:
		...
		for line in indexes_file.readlines():
			...
			index_dictionary[working_list[4]] = [(str(working_list[0])+"_"+str(working_list[1])+"_"+str(working_list[2])+"_"+str(working_list[3])+"_L008_R1_001.fq.gz"),(str(working_list[0])+"_"+str(working_list[1])+"_"+str(working_list[2])+"_"+str(working_list[3])+"_L008_R2_001.fq.gz")]
...
def Read1opener(barcode,L1,L2,L3,L4):
	...
	if barcode == "Unknown":
		f = str(args.output+"Unknown_L008_R1_001.fastq")
	elif barcode == "Swapped":
		f = str(args.output+"Swapped_L008_R1_001.fastq")
	else:
		f = str(args.output+index_dictionary[barcode][0])
	f.write(L1)
	f.write(L2)
	f.write(L3)
	f.write(L4)
...
def Read2opener(barcode,L1,L2,L3,L4):
	...
	if barcode == "Unknown":
		f = str(args.output+'Unknown_L008_R2_001.fastq')
	elif barcode == "Swapped":
		f = str(args.output+'Swapped_L008_R2_001.fastq')
	else:
		f = str(args.output+index_dictionary[barcode][1])
	f.write(L1)
	f.write(L2)
	f.write(L3)
	f.write(L4)
...
for key,value in index_dictionary.items():
	value[0] = gzip.open(str(path+index_dictionary[key][0]),"at")
	value[1] = gzip.open(str(path+index_dictionary[key][1]),"at")
Unknown_R1 = gzip.open(str(path+'Unknown_L008_R1_001.fastq'), "at")
Unknown_R2 = gzip.open(str(path+'Unknown_L008_R2_001.fastq'), "at")
Swapped_R1 = gzip.open(str(path+'Swapped_L008_R1_001.fastq'), "at")
Swapped_R2 = gzip.open(str(path+'Swapped_L008_R2_001.fastq'), "at")
...
while R1line4:
	...
	if Qscore_eval(I1line4,30) == False or Qscore_eval(I2line4,30) == False:
		...
		if I1line2.strip('\n') == rev_comp and I1line2.strip('\n') in index_dictionary.keys():
			Read1opener(index_dictionary[I1line2.strip('\n')],R1_header,R1line2,R1line3,R1line4)
			Read2opener(index_dictionary[I1line2.strip('\n')],R2_header,R2line2,R2line3,R2line4)
			matched_counter += 1
			total_counter += 1
